...
| `rename` | `target` `topic` — rename a topic (`topic_id` + `name`). `target` `file` — rename a file (`file_id` + `name`). `target` `topic_type` — set a topic's type (`topic_id` + `topic_type`, an existing type name from `list`; `""` clears it). Unused fields are `0` / `""`. System topics cannot be renamed or retyped |
