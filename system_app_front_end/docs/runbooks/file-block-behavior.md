...
Project `plan`, `execution`, and `tasks` files share an ordered part structure.
Each part is an inner `header` block. A project part with the same visible text
should appear in all three files and in the same order.

- `plan` gives the macro project map: part names, short descriptions, and the
  overall shape of the project.
- `execution` is more detailed and changes as work happens; it keeps important
  implementation notes per part.
- `tasks` stores concrete missions under the matching part headers.
- `overview` is generated by automation and shows the current project state,
  current part, flagged tasks, recent progress dates, and an ordered part list.

The generated overview may mark the current part with `is_current_part: true`.
This is display metadata only; the header/list text remains the part identity.
