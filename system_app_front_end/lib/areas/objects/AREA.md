...
- Info **field**: formatting + **Connect info…** / **Remove connection** (description). Info **chrome**: Add tag + **Add connection…** (related, infos only). ⌘L in an object field (info / task / table) opens Connect info (description; from info text this also upserts related). Otherwise ⌘L inserts a list. Chrome **Add connection…** stays related-only.
- Description: right-click marked text / caret line in an object field → Connect info… (search by name; empty-title infos are hidden, including ones the graph used to label `Info`) → dark-teal glyphs + 1px underline, hover bubble (stays open while the pointer is on the connected text or the bubble, and the bubble scrolls), double-click / double-tap opens the target info in its file. Right-click a connected span for **Remove connection**. Typing before a connected span moves the underline with those glyphs (anchors remap and PATCH).
