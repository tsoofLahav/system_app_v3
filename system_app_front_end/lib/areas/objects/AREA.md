...
**Shipped (presentation, in files):** list-like task embed; info title/body + tag chips (field: Connect info; chrome: add tag / related connection); table embed (+ chart quality); Move Mode; description underlines + hover bubble + double-click / double-tap to open the info in its file. Task-title description links also show in views.
