...
**View cadence.** Each view is `routine` (repeating) or `one_time` (`layout_config.cadence`). Every section in that view follows it. Complimentary automation tasks may only sit in a **repeating** view. Creating a section also creates an off **section window** automation (see automations `AREA.md`).
