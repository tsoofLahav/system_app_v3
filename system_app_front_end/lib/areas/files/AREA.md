...
| [`editor/edit_conflict.dart`](editor/edit_conflict.dart) | User vs inbound: take inbound unless dirty; any dirty embed makes the file body ask; leftover body overlaps → lookalike; same-object embed payloads still keep-yours / use-agent |
| [`editor/document_three_way.dart`](editor/document_three_way.dart) | Marker-part 3-way (base / local / server). Keeps blank / `[SPACER]` parts — never collapse `\n\n+` or trim parts away |
...
| [`model/marker_super_editor_bridge.dart`](model/marker_super_editor_bridge.dart) | Marker text ↔ Super Editor document. `[SPACER n="N"]` → N empty paragraphs |
...
The built-in **Reports** topic (`is_system`) is also hidden from the live sidebar. The user cannot create or edit it. Leftover-task reports and closed one-time section archives are standing archived files there, opened from Archive.

### Document vs Super Editor (one sync rule)

**Persisted SoT** = marker text (`%%system_app_document v4\n` + body). **Session SoT** = Super Editor `MutableDocument` (undo via SE history). Save = bridge serialize → debounced `PATCH document_json` with `base_revision` (the revision this edit forked from). Server bumps `content_revision` on every body write; stale `base_revision` → 409 + current file → 3-way / lookalike. Embed node ids are stable (`embed:<objectId>`). Object **payloads** stay in the objects area.
...
A pane with `isBrought` is a file **visiting Home** from another topic (UX bring-file). It is the **same file** as on its source topic — one `files` row, one live record in `AppState.filesById`, shown in both places. Membership is `workspaces.home_visit_file_ids`. The mixed Home canvas order (visits among Home’s own files) is `workspaces.home_canvas_file_ids`, shared with the phone. `topics.file_layout` is desktop-only — phone shows every file in that order, with no slots. A visit occupies a layout slot like any other file and can be rearranged with them. Edits save to that file; the ⋯ menu can dismiss that visit without archiving or deleting it. `showFileMenu: false` hides that menu for throwaway hosts (the fill-file snippet dialog).
...
On phone the framed pane fills the screen with a slim peek so the card outline stays visible (swipe). Top and bottom tool bars float on a grey ombre (topic-tinted off Home). Top chrome is split into small icon pills (menu / bring / add) plus topic · file on one line on the solid ombre. Screen structure is locked in UX [`AREA.md` § Phone screen structure](../ux/AREA.md#phone-screen-structure).
...
**Phone caret in/out of objects.** The enter/leave pill is one way in or out. A tap on body text must also leave the object and place the Super Editor caret there ([`bidiCaretTapHandler`](rich_text/rtl/super_editor_bidi_caret.dart) `onBodyTextTap`). Do not let the embed session swallow that caret.
