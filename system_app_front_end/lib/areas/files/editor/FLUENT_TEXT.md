...
4. **Escape** inside any inner field (and **Enter** inside empty task/table structure units) leaves: unfocus the embed field, place SE caret on a **TextNode after** the embed (insert empty paragraph if needed), then `requestFocus` on the next frame so SuperIme opens only against a live node. **Enter** inside info inserts a newline; **Shift+Enter** / **⌘Enter** inside fields also insert a newline.
