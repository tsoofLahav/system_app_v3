...
| Schedule | Three chips: **day**, **week**, **month**. Weekly and monthly calendars toggle — one tap is one day, more taps add more (no separate “a few times” chip). Month shows an every-N-months field locked at 1; unlock it to type 2–12. That interval is counted from the month of the chosen day (`from YYYY-MM`). Weekly / monthly use a calendar next to a matching 24-hour numbered dial (typed hour and minute under the dial); daily is the clock alone. Times are **Israel** (`Asia/Jerusalem`), not UTC |
...
| [`leftover_clear_dialog.dart`](leftover_clear_dialog.dart) | Blocking Was done / I will do it / Skip and report when a window ends with active tasks |
| Agent dialog / AI bar ⋯ | Saved **actions**, not automations |

## Section windows and complimentary tasks

Every named view section gets a `section_window` automation (created off). Start + duration open a window: attention dots on the **sidebar view** and **section header** until the last **active** task is marked done (the dot clears immediately, without waiting for the next poll) or the duration ends. On **iOS**, each of those dots is also a local notification (one per section); the app-icon badge is the count. The badge and those notifications clear when the last active task is marked done. Refreshes are serialized: a section notice is posted only when its id enters the attention set, never reposted merely because tasks remain open. Badge-only updates occur only when the count changes; repeated empty snapshots are also no-ops. Clearing then reopening a section can notify again. This deduplication is per running app session. Regression: `test/automations/section_notification_dedup_test.dart`. When the duration ends with **nothing missed**, done routine (and complimentary) tasks **stay done** until the next window start — no dialog, no recycle. If any task is still **active**, a **blocking center modal** (any screen — not a snackbar; cannot dismiss without choosing) must be answered: **Was done** marks the listed tasks done; **I will do it** extends this occurrence indefinitely until no active tasks remain, without changing the usual schedule; **Skip and report** settles the tasks as skipped and records structured database history. The dialog resolves its own window ID, not whichever pending window happens to be first after polling. After the window ends, a **one-time** section leaves the live view: non-skipped titles prepend onto a standing archived **One-time tasks** file in Reports, memberships drop, and view-only rows soft-archive. `GET /automations` closes expired windows so the 5s poll can raise the modal without waiting for the minute cron. Changing the window’s start or duration clears that cycle: all section tasks go active and the attention dot goes off.
...
The section-window duration is **hours** and **minutes**, each labelled above the field (not as a disappearing hint). The section-window editor uses the same `extraWideWidth` as the regular builder so the calendar and clock are not crushed.

Timing uses locked structured controls rather than free text, so an invalid schedule string cannot be produced. The builder is three framed sections: details (name, scope, day / week / month chips), when (a compact calendar beside a matching 24-hour numbered dial with typed hour and minute, both in this dialog), and steps (a horizontal strip of frames; long-press drag reorders them — the run walks that array in order). Frequency stays in details. Week taps toggle weekdays (`weekly mon,thu 09:00`). Month infers first / second / third / last from the tapped date (a fourth-of-five that is not the last maps to third) and toggles those slots (`monthly first.fri,third.mon 09:00`). Unlock “every how many months” to skip N−1 months from the month of the tap (`monthly 3 last fri 18:00 from 2026-08`); locked stays every month. Flip months to see where it falls later — only cycle months are marked. Daily shows only the clock. `+` under the strip adds a new AI action (the regular create dialog), a saved AI action, or a system step. Choosing **Add to a file** opens the real file editor on a scratch file; the insert bar in that dialog adds objects the same way as a topic file. Save stores that snippet on the step (`document_json` plus cloned `objects`, appended onto the target at run). Tap a frame to edit it. Remove a step with the corner x on its frame, not from inside the step editor. Enabled is a switch on the automations **list** (outermost after edit / run / delete), not in the builder. Create sits under the regular list. The string sent is `daily 08:00`, never `0 8 * * *`.
...
| [`leftover_clear_dialog.dart`](leftover_clear_dialog.dart) | Blocking leftover choices |
...
| [`section_attention_notices.dart`](section_attention_notices.dart) | Which open sections become iOS notifications |
| [`section_attention_notifications.dart`](section_attention_notifications.dart) | iOS local fallback / APNs badge reconciliation (no-op elsewhere) |
| [`schedule_format.dart`](schedule_format.dart) | DSL parse / format (`daily` / `weekly` / `weekly DAY,DAY` / `monthly` / `monthly N … from YYYY-MM`) |
| [`schedule_kind_field.dart`](schedule_kind_field.dart) | Day / week / month chips; month can unlock every N months |
...
## Remote notifications and refresh
`push_registration.dart` registers native iOS installations against the active workspace. Registrations serialize across profile switches; enrollment survives restarts to suppress duplicate local notifications during outages. Once enrolled, `section_attention_notifications.dart` reconciles the native badge without creating local banners. Backend APNs and a separate minute cron deliver attention while the app is closed; setup lives in backend `docs/PUSH_NOTIFICATIONS.md`. AppState pauses polling while backgrounded, refreshes immediately on resume, and refreshes metadata every minute alongside the existing five-second file/task tick. View memberships refresh even when the attention signature is unchanged.
