...
Tapping outside the focused editor (canvas, empty padding, object chrome — not another field) unfocuses it, hides the caret, **clears the mark**, and closes the keyboard, on phone and desktop ([`shell/dismiss_focus_on_outside_tap.dart`](shell/dismiss_focus_on_outside_tap.dart)). The **bottom menu** is the only keep-focus island: insert and other bar tools must stay usable while typing. On phone, the first pill is arrows plus enter/leave (Shift+Enter has no key). Those arrows never mirror in Hebrew.
