...
A topic shows its files in a shape the user picks. **The shape decides which files are on screen** — there is no property on a file that makes it important. File **order** (`files.order_index`, and Home’s mixed canvas on `workspaces.home_canvas_file_ids`) is shared across phone and computer. **Layout** (`topics.file_layout`) is desktop-only.
...
Opening a topic under **Archive** replaces the main pane with [`archive/archive_topic_view.dart`](archive/archive_topic_view.dart) — desktop and phone use the same view. Phone hides the insert bar and uses the bottom bar’s trash for delete-mode. The page title is the topic headline (icon + name, same as the live topic) with `- Archive`. In the sidebar, archive type folders start closed. The built-in **Reports** topic (`is_system`) sits at the top of Archive and is not in the live sidebar; leftover-task reports land there as archived files and cannot be unarchived onto the main page.
...
| Text (bold, italic, underline, strikethrough, Make link, cut/copy/paste, size) | ⌘B/I/U/Y/X/C/V, ⌘⇧+/− | Mark, else the caret line — except **paste**, which inserts at the caret unless something is marked. Embed fields via `runBlockTextAction`; Super Editor via `DocumentEditorController.applyTextAction`. Super Editor’s own Cmd+B / Cmd+I / ⌘V are stripped so catalog toggles once and list paste keeps `-` / `1.` points. Copy/cut of lists include those prefixes. **Make link** is menu-only (⌘K is bring-file); **strikethrough** is ⌘Y. Click / tap opens a persisted URL. **Make list** (menu, or insert list / ⌘L on marked text) gives each newline a point. |
| Insert object | ⌘D info, ⌘T task, ⌘⌥T table, ⌘G graph, ⌘⇧I image, ⌘L list, ⌘E emoji | Active file via `DocumentEditorRegistry`. ⌘T inside an info field inserts an inner checklist line instead of a task-list object. ⌘L is a list unless the caret is in an object field (Connect info). Emoji toggles the insert-bar palette (also on a view, into the focused task title) and inserts into the body or an open object field |
