...
| Insert object | ⌘D info, ⌘T task, ⌘⌥T table, ⌘G graph, ⌘⇧I image, ⌘L list, ⌘E emoji | Active file via `DocumentEditorRegistry`. ⌘L is a list unless the caret is in an object field (Connect info). Emoji toggles the insert-bar palette and inserts into the body or an open object field |
