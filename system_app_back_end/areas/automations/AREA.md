...
A **section window** has no steps. The minute cron opens it at the start and recycles complimentary / routine tasks (unmark). Duration end does **not** recycle: done work stays done until the **next start**. Only leftover **active** tasks write `pending_clear` and wait for a blocking Report / Dismiss. Close must use `window_should_close` (opened + `now >= closes` + no pending payload) — not `window_is_open`, which is already false after close time. `GET /automations` and `GET /automations/pending-clears` also call `close_expired_section_windows` so the client poll can raise the modal without waiting for cron. Saving a new start or duration on an open window calls `clear_section_window_state` (unmark, drop the dot). **Report** prepends the miss (Israel wall time, view / section, task titles) onto a standing **archived** `Missed tasks` file (`meta.system_kind = missed_section_report`) in the built-in **Reports** topic (`topics.is_system`) and leaves those leftover rows **active**. **Dismiss** marks those leftover rows done in place. After the window ends, a **one-time** section leaves the live view: titles prepend onto a standing archived `One-time tasks` file (`meta.system_kind = one_time_section_archive`) in Reports, memberships drop, and view-only rows soft-archive. That topic is not in the live sidebar and cannot be edited or deleted; it only appears under Archive. Standard automations locked to a section copy that schedule and **do not fire on their own clock** if the window is off. If they need user input they wait for `POST /automations/:id/submit-input`; if they only need review they run at section start. Complimentary tasks are created **only for the roles the steps need** — input when `requires_user_input`, review when `apply_mode` is review — never a spare review row on an input-only automation. `GET /automations` prunes leftover roles. List payload includes `has_pending_review` so the review hover is silent until a pending review exists.
