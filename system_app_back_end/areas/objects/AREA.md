...
**Section cadence and windows.** `views.layout_config.cadence` is `routine` (repeating) or `one_time` for the whole view; every `sections[]` entry follows it (`key` still stable, `default` at most one section). PATCH of `layout_config` backfills keys, stamps view cadence onto sections, and creates matching `section_window` automations (automations area). Complimentary tasks (`tasks.source_automation_id`, `complimentary_role`) are marked by the pipeline when the work finishes; the checkbox still toggles like any other task (giving up that round). Only the roles the automation needs are stored. Complimentary tasks may only sit in a repeating view. Duration end with leftover **active** tasks writes `pending_clear` and waits for Was done (done), I will do it (extend this occurrence), or Skip and report (structured history and skipped status). Done routine work stays done until the next start. One-time section tasks leave the live view after the window — see automations `AREA.md`.
