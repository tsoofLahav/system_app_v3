...
| [`routes/ai_actions.py`](routes/ai_actions.py) | Saved AI actions: CRUD, bar order, run on live scope. English `name` + Hebrew `name_he` (both required). Optional `requires_user_input` / `user_input_prompt` so an automation that uses the action places complimentary input/review tasks. Scope: `topic_id` xor `topic_type_id` xor neither (all). At most two actions may be scoped to one topic; those occupy extra seats 9–10 only on that topic. Template topics are never in that scope. Visiting files on Home are a frontend visibility union — the run still uses live `scope` / `hints` |
