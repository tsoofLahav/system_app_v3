...
Home is a built-in working topic, distinct from `is_system` Reports (which is hidden in Archive). Existing clients identify Home by the literal name `Home`. The server identifies its row by the bootstrapped Daily file's `meta.automation_anchor` and restores the `Home` name, no colour, and no type during bootstrap and topic listing. Agent and topic routes reject changes to Home's identity or appearance; layout and order remain editable. This also repairs a Home row whose name was translated by an older agent run.
