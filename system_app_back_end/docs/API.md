...
`file_layout` is desktop presentation (`auto`, `single`, `split`, `hero`, `grid`). Phone has no layouts and shows every file in `order_index` order. `auto` follows file count (1 → single, 2 → split, 3+ → three-file hero) until the user picks a layout. Leftover `hero_left` / `hero_right` / `row` still load. It also decides how many files the computer shows at all — see [files area](../areas/files/AREA.md).
...
| GET | `/home-visits` | Visiting file ids + shared Home `canvas_order` |
| PUT | `/home-visits` | Replace `{ "file_ids", "canvas_order?", "workspace_id?" }` |
