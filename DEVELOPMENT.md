...
- **2026-08-08** — File editor structural edits that still use [`DocumentSession`](system_app_front_end/lib/areas/files/editor/document_session.dart) fold into the buffer via `_commitSessionResult` / `loadFromRichDocument`, then reset controllers. Never coalesce/move/delete and leave stale `TextEditingController`s mounted. File pane is the only scroll owner.
