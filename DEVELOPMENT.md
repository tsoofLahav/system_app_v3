...
- **2026-08-08** — File editor structural edits go through [`DocumentSession`](system_app_front_end/lib/areas/files/editor/document_session.dart) then one `_commitSessionResult` that resets controllers from `_doc`. Never coalesce/move/delete and leave stale `TextEditingController`s mounted. File pane is the only scroll owner.
