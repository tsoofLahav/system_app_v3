...
**2026-09-08 — Document sync:** `DocumentSync` is the per-file owner of baseline body/revision, draft, generation, and in-flight work. Autosave, polling, navigation and agent refresh request the same fetch/reconcile/guarded-write path. Edits during awaits remain dirty. A 409 re-fetches and re-merges; never retry unchanged content with a substituted revision. Apply only a changed displayed snapshot, after keyboard idle and a fresh generation check. Object saves are awaited separately. Exact flow, limits, and regression checks: [`DOCUMENT_FLOW.md`](system_app_front_end/lib/areas/files/editor/DOCUMENT_FLOW.md).
