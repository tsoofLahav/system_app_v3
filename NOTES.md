...
**2026-09-06 — Cross-device body poll:** the 5s `refreshSectionWindows` tick also merges open-topic and brought-file `document_json` (and embeds, keeping dirty payloads) into `filesById`. Super Editor applies via its own listener + `runWhenKeyboardIdle` — do not remount the topic canvas for that. Document PATCH carries `base_revision` (migration `025`); 409 → keep local text and 3-way/lookalike. After lookalike Finish/Discard: **close dialog first**, then busy-wash → remount Editor → PATCH (remounting under the open dialog spam-logs `DiagnosticsProperty` and corrupts the caret). 3-way `splitMarkerParts` must keep blank/`[SPACER]` parts (exact `\n\n`, no trim-drop) — collapsing them made Finish look like it deleted empty lines. When both devices have unsaved work against a differing inbound body, 3-way first; lookalike only on leftover overlaps — never silently prefer the other device. Phone IME has no keys-down, so a dirty gate matters: clean + focused can take inbound after idle; dirty must ask. **Dirty draft:** while the open editor is dirty, poll stashes the server tip in `inboundDocumentJson` and keeps the live draft in `filesById` (updated on each doc change) so refresh/remount cannot drop unsaved blanks; SE reads remote from that stash for 3-way. **Stale poll:** ignore inbound when `inboundRev < localRev` or while PATCH is in flight — a late pre-save body must not remount after save. Stages: [`DOCUMENT_FLOW.md`](system_app_front_end/lib/areas/files/editor/DOCUMENT_FLOW.md).

**2026-09-06 — Agent apply blanks:** do not merge consecutive `[SPACER n="1"]` into `n="2"` on apply (identity rewrite). FE load must expand `[SPACER n="N"]` to N empty paragraphs — treating every SPACER as one blank crushed gaps after Finish even when the model kept the markers. Separate bug: model often writes a bare blank line (`A\n\n\nB`) instead of `[SPACER]`; lookalike still shows the gap, but `_text_to_blocks` used to strip the leftover `\n` and Finish densified the region.
...
**2026-08-26 — Object fields:** Material `TextField`. Native caret. Click placement only (`embedCaretForTap`) — not drags, not Shift+click, not an existing mark (Hebrew padding must not expand to the whole line). Keep-focus on the open object and the bottom menu. Do not overlay-paint a caret or rewrite selection while typing. Do not reseed a focused info field (`setRichState` jumps the caret to the end). `onKeyEvent` once (Arrow Up stack-overflow). One `DocumentTextFlow` per **multi-field** object so Shift+arrows mark across tasks or cells in that object only; info is one field and has no flow. Do not `ensureVisible` object fields, and do not let `EditableText.showOnScreen` walk up to the file pane (the text hops). Description links open on double-click / double-tap, not a single click. Task rows must not change widget key when the new task’s id arrives — that remounts the field and drops focus after Enter. **Handoff:** click/right-click a paragraph forgets the object mark (`releaseLiveMark`); click/right-click an inner field focuses it and clears the Super Editor caret. Super Editor `hasFocus` is true for descendant fields — only **primary** focus means the body owns writing. Never freeze a leftover object mark into the body menu. A **new** right-click retargets the mark and menu even while the previous right-click menu is still open (`beginNewPointerAim`). Full list: [`CARET_AND_WRITING_FOCUS.md`](CARET_AND_WRITING_FOCUS.md) § Writing in objects.

---

## Files and text

- **Document flow (save / poll / remote):** [`DOCUMENT_FLOW.md`](system_app_front_end/lib/areas/files/editor/DOCUMENT_FLOW.md) — stages, stale-poll rule, how to re-enable spacer audits.
- **2026-08-09** — **Marker-text SoT (v4):** files store `%%system_app_document v4` pointer-marker text ([`DOCUMENT_TEXT.md`](system_app_front_end/lib/areas/files/editor/DOCUMENT_TEXT.md)); agent text expands/collapses objects. Legacy v3 JSON migrates on read (spans dropped except web links, stored as `[text](url)`).
...
- **2026-07-27** — **There is only one marking.** Every action (right-click, clipboard, formatting, Make link, and AI) resolves its target through `DocumentMark` in embed fields and `caretLineSelection` in Super Editor body: the marking if there is one — across as many parts as it covers — otherwise the line at the caret. **Paste** does not use that caret-line fallback: unmarked paste inserts at the caret; a marking still replaces. Never read a single field's selection to decide what an action affects.
- **2026-07-27** — **A bullet in a list and a row in a table each count as one line of text.** Settle any caret or marking question by asking what a plain line would do: arrow up lands on the *last* line of what is above, marking a whole row and deleting removes the row, marking every file-body bullet removes the list. Inside a task list or table the object's flow must prune those rows (`onPruneStructures`) — otherwise delete only clears the text. Inner mark-delete must not destroy the object: if every task/row is marked, one empty part stays. Rules in the files [`AREA.md`](system_app_front_end/lib/areas/files/AREA.md).
- **2026-08-26** — **Inner mark-delete keeps the object.** Dropping fully marked tasks/rows from inside a task list or table must not call `onDeleteObject` / `onDeleteTable`. If every part is marked, keep one empty part. Delete the object from chrome or empty Backspace on the last unit. Clearing a disposed `DocumentTextFlow` from `releaseLiveMark` is what stuck the editor when the object unmounted.
...
- **2026-08-23** — Default layout is `auto` (1 → single, 2 → split, 3+ → three-file hero) until the user picks something. Cycle-files (⌘[ ⌘]) rotates every live file in the topic, not only the on-screen band, and does not need a file focused. New file name starts empty.
- **2026-08-26** — Four layouts: grid, one file, two files (half/half), three files with the large pane on the start edge (left in LTR, right in RTL). Layout picker is a glass strip above the bottom bar with no scrim; arrange overlay reorders files only. Legacy `hero_left` / `hero_right` / `row` still load.
