...
**2026-09-06 — Agent apply blanks:** do not merge consecutive `[SPACER n="1"]` into `n="2"` on apply (identity rewrite). FE load must expand `[SPACER n="N"]` to N empty paragraphs — treating every SPACER as one blank crushed gaps after Finish even when the model kept the markers.
